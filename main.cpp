...
// ----------------------------------------------------------
 bool dfs(int r, int c, const vector<vector<int>>& maze, vector<vector<bool>>& visited,
    ...
    //check to see if current position is out of bounds
    ...
    // for loop to check all 4 neighbors (up down left right)
    for (int d = 0; d < 4; ++d) {
        int nr = r + dr[d];
        int nc = c + dc[d];
        //checking guards for before the recursive call
        ...
        // setting parents before recursive call
        parent_r[nr][nc] = r;
        parent_c[nr][nc] = c;
